...
class Solution3
{
    ...
    public static void printArr(List<Object> arr) throws IOException{
        for(Object a : arr)
            printSpace(a);
        println("");
    }
    public static void print2DArr(int[][] dp) throws IOException{
        printSpace("{ ");
        printSpace(
            Arrays.stream(dp)
                .map(a -> String.join(" ", a))
    ...
    static void testCase() throws IOException{
         try{
            ...
            printSpace(logic("babad"));
            println("bab");
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic("cbbd"));
            println("bb");
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
    ...
    static int logic(String str) throws IOException{
        int n = str.length();
        if(n==1)
            return n;
