    ...
    static String logic(String[] strs) throws IOException{
        ...
        for(int i=0; i<strs[0].length(); i++){
            for(int j=i+1; j<=strs[0].length(); j++){
                prefixes.add(strs[0].substring(i,j));
            }
        }
        ...
        String answer = "";
        boolean found = false;
        boolean[] foundArr = new boolean[prefixes.size()];
        for(int i=0; i<prefixes.size(); i++){
            found = true;
            for(int j=1; j<strs.length; j++){
                if(strs[j].indexOf(prefixes.get(i))==-1){
                    found=false;
                    break;
                }
            }
        ...
        for(int i=0; i<prefixes.size(); i++){
            if(foundArr[i] && max_length<prefixes.get(i).length()){
                max_length=prefixes.get(i).length();
                answer = prefixes.get(i);
            }
        }
