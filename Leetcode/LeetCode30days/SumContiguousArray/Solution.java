...
class Solution
{
    ...
    static void testCase() throws IOException{
         try{
            ...
            printSpace(logic(new int[]{0,1}));
            println(2);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new int[]{0,1,0}));
            println(2);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new int[]{0,1,0,1,1,0,1,0,1,0}));
            println(6);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
    ...
    static int logic(int[] nums) throws IOException{
        ...
        for(int i=0; i<2; i++){
            ...
            while(index<nums.length){
                println(index+" "+local_max+" "+isZero);
                if((nums[index]==0 && isZero) || (nums[index]==1 && !isZero)){
                    local_max+=1;
                    isZero=!isZero;
                    index++;
                    //println("su "+index+" "+local_max);
                }else{
