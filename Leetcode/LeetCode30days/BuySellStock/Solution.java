...
class Solution
{
    ...
    static void testCase() throws IOException{
         try{
            ...
            printSpace(logic(new int[]{7,1,5,3,6,4}));
            println(7);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
            
            startTime = System.currentTimeMillis();
            printSpace(logic(new int[]{1,2,3,4,5}));
            println(4);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new int[]{7,6,4,3,1}));
            println(0);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new int[]{7,3,5,1,2,3,4,5,6}));
            println(8);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
    ...
    static int logic(int[] prices) throws IOException{
        int max_profit=0;
        ...
        for(int i=0; i<prices.length-1; i++){
            if(buy==false && prices[i]<prices[i+1]){
                min_index=i;
                buy=true;
            }else if(buy==true && prices[i]>prices[i+1]){
                max_index=i;
        ...
        if(buy==true && (max_index==Integer.MAX_VALUE || min_index>max_index))
            max_profit+=prices[prices.length-1]-prices[min_index];  
