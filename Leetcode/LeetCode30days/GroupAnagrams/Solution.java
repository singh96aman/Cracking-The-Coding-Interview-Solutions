...
class Solution
{
    ...
    static void testCase() throws IOException{
         try{
            ...
            printSpace(logic(new String[]{"eat","tea","tan","ate","nat","bat"}));
            println("[[\"bat\"],[\"nat\",\"tan\"],[\"ate\",\"eat\",\"tea\"]]");
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new String[]{""}));
            println("[[\"\"]]");
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic(new String[]{"a"}));
            println("[[\"a\"]]");
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
    ...
    static List<List<String>> logic(String[] strs) throws IOException{
        ...
        for(String str : strs){
            ...
            if(result.containsKey(hash)){
                List<String> values = result.get(hash);
                values.add(str);
                result.put(hash, values);
            }else{
        ...
        for(int hash : result.keySet())
            finalResult.add(result.get(hash));
        return finalResult;
