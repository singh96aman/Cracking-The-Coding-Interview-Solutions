...
class Solution
{
    ...
    static void testCase() throws IOException{
         try{
            ...
            printSpace(logic("ab#c", "ad#c"));
            println(true);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic("ab##", "c#d#"));
            println(true);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");

            startTime = System.currentTimeMillis();
            printSpace(logic("a#c", "b"));
            println(false);
            stopTime = System.currentTimeMillis();
            println((stopTime - startTime)+" ms");
    ...
    static String convertStrings(String str){
        StringBuilder sb_s = new StringBuilder();
        for(int i=0; i<str.length(); i++){
            if(str.charAt(i)!='#')
                sb_s.append(str.charAt(i));
            else if(sb_s.length()>0)
                sb_s.deleteCharAt(sb_s.length()-1);
        }
        return sb_s.toString();
    }

    static boolean logic(String s, String t) throws IOException{
        int ans = convertStrings(s).compareTo(convertStrings(t));
        if(ans==0)
            return true;
        else
            return false;
